...
namespace OpenglLib
{
    public partial struct ShadowMapComponent : IComponent
    {
        ...
        public uint[] ShadowMapTextureIds;
        public int[] LightIds;
        ...
        {
            Owner = owner;
            ShadowMapTextureIds = new uint[MAX_SHADOW_MAPS];
        ...
        public int AddDirectionalLightShadowMap(int lightId, uint shadowMapId)
        {
            for (int i = 0; i < LightParams.MAX_DIRECTIONAL_LIGHTS; i++)
            {
                if (LightIds[i] == -1)
                {
                    ...
                    ShadowMapTextureIds[i] = shadowMapId;
                    return i;
                }
            }
            return -1;
        ...
        public int AddPointLightShadowMap(int lightId, uint shadowMapId)
        {
            for (int i = LightParams.MAX_DIRECTIONAL_LIGHTS; i < MAX_SHADOW_MAPS; i++)
            {
                if (LightIds[i] == -1)
